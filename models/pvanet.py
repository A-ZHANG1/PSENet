...
class PVANetFeat(nn.Module):
    # This class is im
    def __init__(self, num_classes, scale = 1):
    ...
    def forward(self, x):
        ...
        # h = self.relu1(h)
        # h = self.maxpool(h)
        c2 = self.conv12(x0)         # 1/4 feature
        c3 = self.conv13(c2)         # 1/8
        c4 = self.conv14(c3)         # 1/16
        c5 = self.conv15(c4)         # 1/32
        # return x4
        # Top-down
        p5 = self.toplayer(c5)
...
class PVANet(nn.Module):
    def __init__(self, inputsize=224, num_classes=1000):
        ...
        self.features = PVANetFeat(num_classes)
